    ...
    use solana_sdk::{
        signature::{Keypair, Signer, Signature as Sgnt},
...
pub fn airdop(client: &RpcClient, key: &Keypair)-> Result<Sgnt, Box<dyn std::error::Error>> {
    ...
    pub fn transfer_sol(client: &RpcClient, to: &Keypair, from: &Keypair)-> Result<Sgnt, Box<dyn std::error::Error>> {
    ...
    pub fn empty(client: &RpcClient, to: Keypair, from_keypair: Keypair) -> Result<Sgnt, Box<dyn std::error::Error>> {
